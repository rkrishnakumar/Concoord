  ...
  try {
    ...
    const tokenResponse = await axios.post(
      ...
      new URLSearchParams({
        ...
        code: code,
        redirect_uri: `http://localhost:3000/api/auth/acc/callback`,
    ...
    await db.accCredentials.update({
      where: { userId: session.user.id },
      data: {
    ...
    return NextResponse.redirect(new URL('/home?success=acc_authenticated', request.url))
  } catch (error) {
    console.error('Error in ACC OAuth callback:', error)
