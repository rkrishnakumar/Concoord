  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return NextResponse.json(
        { error: 'Procore credentials not configured. Please contact support.' },
        { status: 500 }
      )
    }
    ...
    // Build authorization URL for Procore
    const redirectUri = `${process.env.NEXTAUTH_URL}/api/auth/procore/callback`
    const authUrl = new URL('https://login.procore.com/oauth/authorize')
    authUrl.searchParams.set('response_type', 'code')
    authUrl.searchParams.set('client_id', clientId)
    authUrl.searchParams.set('redirect_uri', redirectUri)
    // Procore might not require scope parameter or use different format
