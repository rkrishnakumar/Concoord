...
export default function EditSyncPage({ params }: { params: { id: string } }) {
  ...
  const loadSyncData = async () => {
    try {
      setLoading(true)
      const response = await apiFetch(`/api/syncs/${syncId}?userId=${session?.user?.id}`)
      ...
      if (response.ok) {
        ...
        setSelectedSourceSystem(sync.sourceSystem)
        setSelectedDestinationSystem(sync.destinationSystem)
        setSelectedSourceDataTypes(sync.sourceDataTypes || ['issues'])
        setSelectedDestinationDataTypes(sync.destinationDataTypes || ['issues'])
        
        // Set project selections
        if (sync.sourceSystem === 'acc') {
          setSelectedAccProject(sync.sourceProjectId)
        } else if (sync.sourceSystem === 'procore') {
          setSelectedProcoreProject(sync.sourceProjectId)
        } else if (sync.sourceSystem === 'revizto') {
          setSelectedReviztoProject(sync.sourceProjectId)
        }
        
        if (sync.destinationSystem === 'procore') {
          setSelectedProcoreCompany(sync.destinationCompanyId || '')
          setSelectedProcoreProject(sync.destinationProjectId)
        } else if (sync.destinationSystem === 'acc') {
          setSelectedAccProject(sync.destinationProjectId)
        } else if (sync.destinationSystem === 'revizto') {
          setSelectedReviztoProject(sync.destinationProjectId)
        }
        
        // Set field mappings
        setFieldMappings(sync.fieldMappings || {})
        
        setMessage('Sync loaded successfully!')
      } else {
        const error = await response.json()
        setMessage(`Failed to load sync: ${error.error}`)
      }
    } catch (error) {
      console.error('Error loading sync:', error)
      setMessage('Failed to load sync')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage('✅ Sync updated successfully! Redirecting to home...')
        setTimeout(() => {
          window.location.href = '/home'
        }, 2000)
      } else {
        const error = await response.json()
        setMessage(error.error || 'Failed to update sync')
      }
    } catch (error) {
      setMessage('Error updating sync')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <PageLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Edit Sync</h1>
      </div>
